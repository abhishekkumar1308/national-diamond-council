    ...
    async sendOtp() {
      try {
        ...
        if (response?.data?.status) {
          if (!this.authStore.contractor.name || this.authStore.contractor.mobile!==this.mobile) {
            this.authStore.loginContractor({userId: 0, name: this.name, mobile: this.mobile});
          }
          this.$router.push({
